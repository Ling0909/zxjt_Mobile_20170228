...
public class TestE02ETFWXGPRG extends TestBase {
	...
	public void testETFWXGPRG(Map<String, String> param) {
		...
		// 输入代码并校验名称
		String vActualStockName = mPage.doInputCode(param.get("ETF代码"));
		String vExpectStockName = param.get("ETF名称");
		AssertUtil.assertEquals(vExpectStockName, vActualStockName);

		String vActualCFGName = mPage.doInputCFGCode(param.get("成分股代码"));
		String vExpectCFGName = param.get("成分股名称");
		AssertUtil.assertEquals(vExpectCFGName, vActualCFGName);
